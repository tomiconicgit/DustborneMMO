// file: src/game/objects/GoldOre.js
...
// Shared "currently mined ore" across ALL ore types.
// (Prevents copper and gold from mining at the same time.)
const ActiveOre = {
  get current() { return window.__ActiveOre || null; },
  set current(v) { window.__ActiveOre = v; }
};

export default class GoldOre {
  static all = new Map(); // mesh.uuid -> instance

  static createFromMesh(mesh, tileX, tileZ) {
    const ore = new GoldOre(mesh, tileX, tileZ);
    GoldOre.all.set(mesh.uuid, ore);
    return ore;
  }
  ...
  constructor(mesh, tileX, tileZ) {
    ...
    // Same balance as copper for now
    this.maxHealth = 3;
    ...
    // Tag children so ray hits resolve to this ore
  ...
  onTapped() {
    ...
    // Nearest 4-way adjacent standing tile
    const cands = [
  ...
  _applyMiningHit() {
    ...
    // Placeholder loot: +1–2 Gold into slot #1 (second slot)
    const gained = 1 + Math.floor(Math.random() * 2);
    const slot = document.querySelector('.inv-grid .inv-slot[data-index="1"]');
    if (slot) {
